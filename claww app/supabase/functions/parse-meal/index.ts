...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Estimate-only — does NOT write to meal_logs. The client shows this to
    // the user for review/edit first (see app/lib/data.ts's saveMeal), the
    // same confirm-before-log pattern already used for workout sets. Saving
    // directly here meant a bad estimate landed in the log with no chance to
    // correct it before it was already there.
    return new Response(
      JSON.stringify({
