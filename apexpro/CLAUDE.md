...
- [~] SHIP PHASE 6 — Prove the engine is alive. **Gates everything else.** `_shared/groq.ts` documents its own model returning `403 model_permission_blocked_org`; if that is still true in production, every user is silently served the static `DEFAULT_PLAN` and the app's value proposition is fictional. Verify, add a health endpoint, record every fallback, add a model fallback chain
  - [ ] 6.1 Confirm generation works end-to-end in production — **blocked on live access**: needs either the deployed project's Edge Function logs or its `HEALTH_CHECK_SECRET` + URL to call 6.2's endpoint directly. Not yet verified against the real project; do not assume 6.2-6.4 shipping means the underlying model access problem is actually fixed
  - [x] 6.2 `health-check` Edge Function (`supabase/functions/health-check/index.ts`) — probes every model in both chains directly via `probeGroqModel` (bypasses the 6.4 fallback so each candidate's real status is visible), secret-guarded via `HEALTH_CHECK_SECRET` (not a user JWT — deploy with `--no-verify-jwt`), no cap burn since it never calls `enforceGenerationCap`. Code-reviewed and typechecked (via a Deno-globals-stubbed `tsc` pass, since this repo has no Deno runtime installed — see below); not yet deployed or called against a real Supabase project
  - [x] 6.3 `generation_failures` table added to `database/schema.sql` (idempotent, RLS'd own-rows-only like every other table), plus `_shared/generationFailures.ts` (`logGenerationFailure`, `classifyGroqFailure`) wired into every fallback path in `generate-plan`, `parse-meal`, and `parse-meal-photo` — schema-validation failures, grounding-emptied plans, and every Groq error class, not just permission blocks. 3 new tests in `generationFailures.test.ts` covering the full kind-mapping table. Not yet run against a live database — the migration needs to actually execute before this table exists anywhere
  - [x] 6.4 Ordered fallback chains in `_shared/groq.ts` (`GROQ_TEXT_MODELS`, `GROQ_VISION_MODELS`) — `callWithFallback` tries each candidate in turn on a retryable failure (permission-blocked, not-found, rate-limited, or 5xx) and aborts immediately on a non-retryable one (e.g. malformed request, since every model would reject it identically); `DEFAULT_PLAN` remains the final backstop if every candidate fails. 4 new tests in `groq.test.ts` with a mocked `fetch` verifying the actual fall-through behavior (blocked→success, rate-limited+not-found→success, all-blocked→throws-last-error, non-retryable→no-retry), not just that the code typechecks. **Model list itself is unverified** — see 6.1
  - **Still needed to fully close this phase:** run `database/schema.sql` against the live project, deploy the 3 updated functions + the new `health-check` function, set `HEALTH_CHECK_SECRET`, then run 6.1's curl check for real
